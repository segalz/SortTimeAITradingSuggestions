...
- [x] **2.4 Evaluation Metrics & Runner**
  - [x] 2.4.1 Implement forecast error metrics (MAE, RMSE, Directional Accuracy) in `src/trading_engine/evaluation/metrics.py`.
  - [x] 2.4.2 Implement Benjamini-Hochberg multiplicity correction in `src/trading_engine/evaluation/stats.py`.
  - [x] 2.4.3 Create `HarnessRunner` executing baselines across rolling cutoffs in `src/trading_engine/evaluation/harness.py`.
  - [x] 2.4.4 Create tests in `tests/test_harness.py` verifying metric calculation on synthetic series.
- [x] **2.5 Stage 2 Review & Hardening Gate**
  - [x] 2.5.1 Run all leakage tests and verify pass.
  - [x] 2.5.2 Grok audit and sign-off report in `docs/stage2_signoff.md`.
